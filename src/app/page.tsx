    ...
    <div className={`container mx-auto px-4 py-16 ${poppins.className}`}>
      ...
      <section className="bg-white p-10 rounded-3xl shadow-lg mb-20">
        ...
        <div className="grid md:grid-cols-4 gap-6">
          <ImpactCard title="Waste Collected" value={`175 kg`} icon={Recycle} />
          <ImpactCard title="Reports Submitted" value={50} icon={MapPin} />
          <ImpactCard title="Tokens Earned" value={200} icon={Coins} />
          <ImpactCard title="CO2 Offset" value={`${26} kg`} icon={Leaf} />
        </div>
      </section>
    </div>
  )
...
function ImpactCard({ title, value, icon: Icon }: { title: string; value: string | number; icon: React.ElementType }) {
  ...
  return (
    <div className="p-6 rounded-xl bg-gray-50 border border-gray-100 transition-all duration-300 ease-in-out hover:shadow-2xl">
...
function FeatureCard({ icon: Icon, title, description }: { icon: React.ElementType; title: string; description: string }) {
  return (
    <div className="bg-white p-8 rounded-xl shadow-md hover:shadow-lg transition-all duration-300 ease-in-out flex flex-col items-center text-center">
      <div className="bg-green-100 p-4 rounded-full mb-6 hover:scale-105">
